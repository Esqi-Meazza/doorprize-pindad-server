...
const queryConn = (conn, sql, params) =>
  new Promise((resolve, reject) => {
...
const executeSpin = async (id_kelompok) => {
  ...
  try {
    ...
    for (let i = 0; i < finalWinnerCount; i++) {
      ...
      await queryConn(connection, `UPDATE users SET status_menang = 'sudah' WHERE id_user = ?`, [
        user.id_user,
      ]);
      hasilPemenang.push({
...
const undoSpin = async (id_kelompok) => {
  ...
  try {
    ...
    const pemenang = await queryConn(
      connection,
    ...
    await queryConn(connection, `DELETE FROM pemenang WHERE id_kelompok = ?`, [id_kelompok]);
...
const setActiveSessionDB = async (id_kelompok) => {
  ...
  try {
    ...
    await queryConn(
      connection,
      `UPDATE kelompok_hadiah SET status_sesi = 'pending' WHERE status_sesi = 'active'`,
    );

    if (id_kelompok) {
      await queryConn(
        connection,
        `UPDATE kelompok_hadiah SET status_sesi = 'active' WHERE id_kelompok = ?`,
        [id_kelompok],
      );
    }
...
const getActiveSessionFromDB = async () => {
  ...
  try {
    const rows = await queryConn(
      connection,
      `SELECT * FROM kelompok_hadiah WHERE status_sesi = 'active' LIMIT 1`,
      [],
    );
    connection.release();
    return rows[0] || null;
...
const moveToNextSession = async (current_id) => {
  ...
  try {
    ...
    await queryConn(
      connection,
      `UPDATE kelompok_hadiah SET status_sesi = 'complate' WHERE id_kelompok = ?`,
      [currentId],
    );

    let nextSession = await queryConn(
      connection,
...
const getAllSessions = async () => {
  const connection = await getConnection();
  try {
    const sessions = await queryConn(
      connection,
      `SELECT * FROM kelompok_hadiah ORDER BY id_kelompok ASC`,
      [],
    );
    connection.release();
    return sessions;
...
module.exports = {
  ...
  moveToNextSession,
  getAllSessions,
  __test: { pickPrizesWeighted, pickEligibleUsers },
};
